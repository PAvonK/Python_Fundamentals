# -*- coding: utf-8 -*-
"""
Created on Sun Feb  9 21:28:00 2020
...
off the debt within a year. Try it out with large inputs, and notice how fast 
it is (try the same large inputs in your solution to Problem 2 to compare!). 
...
def lowestPayment(balance, annualInterestRate):
    ...
    upperBound = balance + balance * annualInterestRate
    guess = (lowerBound + upperBound) / 2
    guessCheck = 1
        
    while guessCheck > .01 or guessCheck < -.01:
        for i in range(12):
        ...
        if balance < abs(epsilon):
            ...
            lowerBound = guess
        elif (balance + initBalance) /2 <= abs(epsilon):
            break
        guess = (lowerBound + upperBound) / 2
        guessCheck = balance
        balance = initBalance
